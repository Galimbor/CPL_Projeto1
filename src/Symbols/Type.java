...
public class Type {
    ...
    public static boolean isConvertibleTo(Type from, Type to)
    {
        ...
        if(from.isPointer())
        {
            ...
            if(from.getPrimitiveType() == PType.VOID && to.isPointer()) return true;
        }
        else
        {
            if(from.getPrimitiveType() == PType.INT && to.getPrimitiveType() == PType.FLOAT) return true;
        }
        return false;
    }
    ...
    public Type(String type)
    {
        if(type.contains("null")) {
            this.isPointer = true;
            this.type = PType.VOID;
        }
        else {
            this.isPointer = type.contains("<");
            if(this.isPointer)  type = type.substring(1,type.length() - 1);
    ...
    {
        return type.equals(new Type(PType.FLOAT)) || type.equals(new Type(PType.INT));
    }


    public static boolean isPrimitive(Type e1)
    {
        return e1.getPrimitiveType() == Type.PType.FLOAT ||
                e1.getPrimitiveType() == PType.BOOL ||
                e1.getPrimitiveType() == Type.PType.STRING ||
                e1.getPrimitiveType() == Type.PType.INT;
    }

    public static boolean isAPrimitivePointer(Type type)
    {
        return type.getPrimitiveType() == PType.BOOL || type.getPrimitiveType() == PType.INT || type.getPrimitiveType() == PType.FLOAT
                || type.getPrimitiveType() == PType.STRING ;
    }
